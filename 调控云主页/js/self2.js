...
$(document).ready(function() {
  ...
  /* $(".first_menu_title").click(function(){
	...
	$(".first_level").click(function(){
			...
			$(this).siblings().children(".first_menu_title").css("background","#e0eef5");
			$(this).siblings().children(".first_menu_title").children(".first_level_title").css("color","#1d4a96");
			$(this).children(".first_menu_title").css("background","#3a8dda");
			$(this).children(".first_menu_title").children(".first_level_title").css("color","#fff");
	})
	////////////////////弹出框效果
	...
	//图标切换
	function set_icon(this_index)
	{
		for( var i=0; i<$('.first_level').length; i++)
		{
			$('.first_level').eq(i).find('.fir').find('img').attr('src','image/no_sam_'+i+'.png');	
			
		}
		$('.first_level').eq(this_index).find('.fir').find('img').attr('src','image/sam_'+this_index+'.png');
		//alert(this_index);
	}
});
////////////跳转
